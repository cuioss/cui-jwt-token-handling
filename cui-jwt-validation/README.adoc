...
The `HttpJwksLoaderConfig` (configured via `HttpWellKnownResolver`) is then used with `IssuerConfig` to set up the `TokenValidator`. It's crucial to use the issuer identifier obtained from the `HttpWellKnownResolver` for the `expectedIssuer` in `IssuerConfig` to ensure consistency.
...
// And wellKnownResolver is available

String expectedIssuerFromDiscovery = wellKnownResolver.getIssuer().getUrl().toString();
