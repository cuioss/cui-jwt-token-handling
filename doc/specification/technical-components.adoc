...
* Issuer URL (configured value, may be overridden by well-known discovery)
* Expected audience values
* Expected client ID values
* JWKS configuration (one of the following):
** HTTP JWKS configuration (httpJwksLoaderConfig) - supports well-known discovery
** File path to JWKS file (jwksFilePath)
** In-memory JWKS content (jwksContent)
* Algorithm preferences
* Custom claim mappers

==== Issuer Identification and Well-Known Discovery

The IssuerConfig supports dynamic issuer identification through a delegation pattern:

* **Configured Issuer**: The `issuer` field provides the base issuer identifier
* **Effective Issuer**: The `getEffectiveIssuer()` method returns the authoritative issuer identifier for token validation
* **Delegation Logic**: When using HTTP JWKS configuration with well-known discovery, the effective issuer is retrieved from the OpenID Connect discovery document via the JwksLoader

This approach ensures that when well-known discovery is used, the issuer identifier from the discovery document takes precedence over the configured value, providing automatic issuer identification for dynamic environments.

Example usage:
[source,java]
----
// The effective issuer may differ from the configured issuer when using well-known discovery
String tokenIssuer = decodedToken.getIssuer();
String effectiveIssuer = issuerConfig.getEffectiveIssuer();
if (effectiveIssuer.equals(tokenIssuer)) {
    // Validation proceeds
}
----
...
* Dynamic issuer identification through well-known discovery

The TokenValidator manages multiple IssuerConfig instances, inspects JWT tokens, determines their issuer using the effective issuer identifier, and selects the appropriate configuration for validation.
