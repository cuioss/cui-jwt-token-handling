...
* Default maximum string size of 1KB (`DEFAULT_MAX_STRING_SIZE`)
...
TokenValidator validator = new TokenValidator(
    ParserConfig.builder()
        ...
        .maxStringSize(1 * 1024)   // 1KB - individual JSON strings
...
* `DEFAULT_MAX_STRING_SIZE = 1 * 1024` (1KB) - individual JSON string values
...
* **maxStringSize (1KB)**: Applied to individual JSON string fields during parsing, preventing any single field from dominating the payload size
