...
/**
 * Test profile with invalid issuer configuration (missing required fields).
 * <p>
 * This profile is used to test the error handling path where the issuer configuration
 * is missing required fields (no JWKS configuration and no public key location).
 */
public class InvalidIssuerConfigTestProfile implements QuarkusTestProfile {

    @Override
    public Map<String, String> getConfigOverrides() {
        Map<String, String> config = new HashMap<>();

        // Default issuer configuration with missing required fields
        config.put("cui.jwt.issuers.default.url", "https://example.com/auth");
        config.put("cui.jwt.issuers.default.enabled", "true");
        // Missing both public-key-location and JWKS configuration
